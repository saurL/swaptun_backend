...
mod tests {
    ...
    use serde_json;
    use swaptun_api::api;
    use swaptun_services::RegisterFcmTokenRequest;
    ...
    #[actix_web::test]
    async fn test_notification_register_fcm_token_success() {
        ...
        let register_request = RegisterFcmTokenRequest {
            token: "test_fcm_token".to_string(),
            device_id: Some("test_device_id".to_string()),
            platform: Some("android".to_string()),
        };

        let req = test::TestRequest::post()
            .uri("/api/notifications/fcm-token")
